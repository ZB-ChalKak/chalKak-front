...
import { myKeywordsState, seasonState, weatherState } from "@/utils/atoms";
...
const Main = () => {
  ...
  const [myKeywords, setMyKeywords] = useRecoilValue<number[]>(myKeywordsState);
  console.log("mykewords", myKeywords);
      ...
      <div className="max-auto">
        ...
        <div className="mt-5 h-auto grid grid-cols-2 gap-4 px-4">
          {(selectedKeyword === "전체"
            ...
            : data?.pages.flatMap((page) => page.posts.filter((post) => post.staticKeywords.includes(selectedKeyword)))
          )?.map((post, index) => <PostContent key={index} post={post} />)}
        </div>
      </div>
    </div>
  );
